...
int main(int argc, char ** argv)
{
  ...
  //if using -b 1, begin_file will be file0001
    ...
    //std::cout<<"number_of_particles "<<number_of_particles<<std::endl;
    			sprintf(outfile, "VolumeFraction_vs_Time");  //create output file
    ...
    sprintf(outfile_1, "Wall_Distance");  //create output file
    ...
    for (int g=begin_file; g<end_file; g++)
{
    ...
    VolumeFraction = Volumes/TotalVolume;
    ...
    fclose(fpout);
    fclose(fpout_1);
    std::cout<<"particle total volume is  "<<Volumes<<" wall_bottom "<<wall_bottom<< " wall_top "<<wall_up<<" Volume Fraction  "<<VolumeFraction <<std::endl;
    std::cout<< "Time & Particle Volume Fraction "<<std::endl;
